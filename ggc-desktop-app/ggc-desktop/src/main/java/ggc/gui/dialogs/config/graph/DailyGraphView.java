...
package ggc.gui.dialogs.config.graph;
...
public class DailyGraphView extends JFAbstractGraphView
{
    ...
    public DailyGraphView(ColorSchemeH cs, DailyValues dv, GlucoseUnitType glucoseUnitType)
    {
        super();
        colorScheme = cs;
        ...
        m_chart = ChartFactory.createTimeSeriesChart(null, translator.getMessage("AXIS_TIME_LABEL"),
            String.format(translator.getMessage("AXIS_VALUE_LABEL"), glucoseUnitType.getTranslation()), BGDataset, true,
            true, false);
    ...
    protected void drawFramework(JFreeChart _chart)
    {
        ...
        defaultRenderer.setSeriesPaint(0, dataAccess.getColor(colorScheme.getColor_bg()));
        insBURenderer.setSeriesPaint(0, dataAccess.getColor(colorScheme.getColor_ch()));
        insBURenderer.setSeriesPaint(1, dataAccess.getColor(colorScheme.getColor_ins1()));
        insBURenderer.setSeriesPaint(2, dataAccess.getColor(colorScheme.getColor_ins2()));
    ...
    protected void drawValues(JFreeChart chart)
    {
        ...
        TimeSeries ins1Series = new TimeSeries(translator.getMessage("BOLUS_INSULIN_SHORT"), Hour.class);
        TimeSeries ins2Series = new TimeSeries(translator.getMessage("BASAL_INSULIN_SHORT"), Hour.class);
        ...
        for (int i = 0; i < data.getRowCount(); i++)
        {
            ...
            if (row.getBG(glucoseUnitType) > 0)
            {
                ...
                {
                    BGSeries.add(time, row.getBG(glucoseUnitType));
                }
                else
                {
                    BGSeries.addOrUpdate(time,
                        MathUtils.getAverage(row.getBG(glucoseUnitType), BGSeries.getDataItem(time).getValue()));
                }
            }
