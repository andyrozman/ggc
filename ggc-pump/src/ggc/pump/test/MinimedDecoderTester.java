...
public class MinimedDecoderTester
{
    ...
    private void decodeCurrentTimeStamp(int ai[], int start)
        ...
        GregorianCalendar gc = new GregorianCalendar(convertYear(y)-1, m-1, ai[4], ai[0], ai[1], ai[2]); //, ai[3]);
        gc.setTimeZone(TimeZone.getTimeZone("CET"));
        
        //m_timeStamp = createTimestamp(j1, i1, l, i, j, k);
//        log.info( (new StringBuilder()).append("decodeCurrentTimeStamp: current time stamp for device is ").append(m_timeStamp).toString());
        ...
        System.out.println(hu.getIntArrayShow(ai));
            ...
            System.out.println("Start: " + start + "   " + gc.get(GregorianCalendar.DAY_OF_MONTH) + "." + gc.get(GregorianCalendar.MONTH) + "." +  gc.get(GregorianCalendar.YEAR) + " " + gc.get(GregorianCalendar.HOUR_OF_DAY) + ":"+ gc.get(GregorianCalendar.MINUTE) + ":"+ gc.get(GregorianCalendar.SECOND));
  ...
  //          log.info( (new StringBuilder()).append("decodeCurrentTimeStamp: current time stamp for device is ").append(m_timeStamp).toString());
    ...
    final Date createTimestamp(int i, int j, int k, int l, int i1, int j1)
            throws BadDeviceValueException
        {
            ...
            GregorianCalendar gregoriancalendar = new GregorianCalendar(Util.convertYear(k), j - 1, i, l, i1, j1);
...
//        HexUtils hu = new HexUtils();
            ...
            try
            {
                ...
                */
                ...
                if ((obj instanceof CommandHistoryData) && (!(obj instanceof CommandReadSensorHistoryData)))
                {
    ...
    {
        int nr = 0;
        DataReaderWriter drw = new DataReaderWriter("/mnt/f/My_Projects/GNUGlucoCenter/Tools/PumpsTool/test_data/roman/mm_history_cmd_128_page_" + nr +".data");
