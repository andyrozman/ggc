 ...
 *
 *  Filename:     AnimasBaseDataConverter
 *  Description:  Converter for basic Animas Data
 *
 *  Author: Andy Rozman {andy@atech-software.com}
 */

public class AnimasBaseDataConverter extends AnimasAbstractDataConverter
{

    public static final Log LOG = LogFactory.getLog(AnimasBaseDataConverter.class);
    ...
    public AnimasBaseDataConverter(AnimasDeviceReader deviceReader, AnimasDeviceData data)
    {
...
//                break;
    ...
    {
        int code = AnimasUtils.createIntValueThroughMoreBits(packet.getReceivedDataBit(10), packet.getReceivedDataBit(11));
        int subCode = AnimasUtils.createIntValueThroughMoreBits(packet.getReceivedDataBit(12), packet.getReceivedDataBit(13));
        ...
        else
        {
            ...
            else
            {
                switch (code)
                {
                    ...
                    default:
                        {
                            packet.addPreparedData("Alarm_Other", dateTime, codes);
    ...
    private void decodeBolusLog(AnimasDevicePacket packet, ATechDate dateTime)
    {
        ...
        bolusEntry.createPreparedData(packet);

        this.data.addBolusLogEntry(bolusEntry);

        // prepare entry for writing
        //System.out.println(bolusEntry);
    }


    private void decodeExtendedBolus(AnimasDevicePacket packet)
    {
        //AnimasUtils.debugHexData(true, packet.dataReceived, packet.dataReceived.size(), "BolusExt: %s [%s]", LOG);
        ...
        ebs.bg = ((int) Math.round(AnimasUtils.createIntValueThroughMoreBits(packet.getReceivedDataBit(12),
                packet.getReceivedDataBit(13)) * 0.1D * BGmult));
        ...
        ebs.iobValue = AnimasUtils.createBigDecimalValueThroughMoreBits(packet.getReceivedDataBit(18),
                packet.getReceivedDataBit(19)).divide(bigDecimals.get("BIG_DECIMAL_100")).intValue();
        ...
        ebs.bolusEntry = data.getBolusLogBySyncRecordId(ebs.syncRecordId);

        ebs.createPreparedData(packet);
    ...
    {
        AnimasUtils.debugHexData(true, packet.dataReceived, packet.dataReceived.size(), "Basal: %s [%s]", LOG);
        ...
        this.data.addBasalLog(dateTime, rate.floatValue(), flag);
    }
    ...
    private void decodeSuspendHistory(AnimasDevicePacket packet, ATechDate dateTime)
    {
        ...
        ATechDate dt = decodeDateTimeFromRawComponents(packet.getReceivedDataBit(11), packet.getReceivedDataBit(10), packet.getReceivedDataBit(12),
            packet.getReceivedDataBit(13));

        if (dt != null)
        {
            packet.addPreparedData("Event_Basal_Run", dt);
        }
    }
    ...
    private void decodeTotalDailyDoseHistory(AnimasDevicePacket packet, ATechDate dateTime)
    {
        ...
        BigDecimal tdd = AnimasUtils.createBigDecimalValueThroughMoreBits(packet.getReceivedDataBit(10), packet.getReceivedDataBit(11),
            packet.getReceivedDataBit(12), packet.getReceivedDataBit(13)).divide(bigDecimals.get("BIG_DECIMAL_10000f"), 3,
            BigDecimal.ROUND_CEILING);

        packet.addPreparedData("TDD_All_Insulin", dateTime, String.format("%4.3f", tdd.floatValue()));

        BigDecimal basal = AnimasUtils.createBigDecimalValueThroughMoreBits(packet.getReceivedDataBit(14), packet.getReceivedDataBit(15),
            packet.getReceivedDataBit(16), packet.getReceivedDataBit(17)).divide(bigDecimals.get("BIG_DECIMAL_10000f"), 3,
            BigDecimal.ROUND_CEILING);

        packet.addPreparedData("TDD_Basal_Insulin", dateTime, String.format("%4.3f", basal.floatValue()));
    }

    private void decodePrimeLog(AnimasDevicePacket packet, ATechDate dateTime)
    {
        BigDecimal primeAmount = AnimasUtils.createBigDecimalValueThroughMoreBits(packet.getReceivedDataBit(10), packet.getReceivedDataBit(11))
        ...
        if (flag == 2)
        {
            packet.addPreparedData("Event_PrimeInfusionSet", dateTime, //
                String.format("%4.3f", primeAmount.floatValue()));
        }
        else if (flag == 3)
        {
            packet.addPreparedData("Event_FillCannula", dateTime, //
                String.format("%4.3f", primeAmount.floatValue()));
        }
        else
        {
    ...
    {
        // THIS METHOD IS COPIED FROM ORIGINAL CODE, BUT IT DOESN'T WORK, AT LEAST NOT AS IT
        // SHOULD. IT IS UNUSED FOR NOW.

        // bg Log: 02 b2 44 49 00 00 b6 17 0f 05 7d 00  [2 178 68 73 0 0 182 23 15 5 125 0 ]

        //AnimasUtils.debugHexData(true, readBuffer, readBuffer.length-2, "bg Log: %s [%s]", LOG);
        ...
        int timeDelta = packet.getReceivedDataBit(startx) + (packet.getReceivedDataBit(startx+1) * 256) + (packet.getReceivedDataBit(startx+2) * 256 * 256);
        ...
        if (timeDelta > 0)
        {
            ...
            int bgValue = packet.getReceivedDataBit(startx+3) + ((short) (packet.getReceivedDataBit(startx+4) & 0x3) * 256);

            //int bgValue = ((int) Math.round((readBuffer[startx+3] + (readBuffer[startx+4] * 256)) * 0.1D * BGmult));
            ...
            if (bgTestFlag == 0)
            {
                System.out.println(String.format("Time: %s, bg=%s",
                        getDateTimeFromCalendar(bgTime),
                        bgValue
                ));
            }
    ...
    {
        this.data.setSoundVolume(AnimasSoundType.Normal, SoundValueType.getSoundValueTypeByCode(packet.getReceivedDataBit(6)));
        this.data.setSoundVolume(AnimasSoundType.AudioBolus, SoundValueType.getSoundValueTypeByCode(packet.getReceivedDataBit(7)));
        this.data.setSoundVolume(AnimasSoundType.TempBasal, SoundValueType.getSoundValueTypeByCode(packet.getReceivedDataBit(8)));
        this.data.setSoundVolume(AnimasSoundType.Alerts, SoundValueType.getSoundValueTypeByCode(packet.getReceivedDataBit(9)));
        this.data.setSoundVolume(AnimasSoundType.Reminders, SoundValueType.getSoundValueTypeByCode(packet.getReceivedDataBit(10)));
        this.data.setSoundVolume(AnimasSoundType.Warnings, SoundValueType.getSoundValueTypeByCode(packet.getReceivedDataBit(11)));
        this.data.setSoundVolume(AnimasSoundType.Alarms, SoundValueType.getSoundValueTypeByCode(packet.getReceivedDataBit(12)));

        if (this.data.isModelPingOrHigher())
        {
            this.data.setSoundVolume(AnimasSoundType.RemoteBolus,
                SoundValueType.getSoundValueTypeByCode(packet.getReceivedDataBit(13)));
        }
    ...
    private void decodeAdvancedSettings(AnimasDevicePacket packet)
    {
        ...
        pumpSettings.maxBasalAmountProHour = AnimasUtils.createBigDecimalValueThroughMoreBits(
            packet.getReceivedDataBit(12), packet.getReceivedDataBit(13)).divide(this.bigDecimals.get("BIG_DECIMAL_100f"), 2,
            BigDecimal.ROUND_CEILING);
        pumpSettings.maxBolusProHour = AnimasUtils.createBigDecimalValueThroughMoreBits(packet.getReceivedDataBit(14),
            packet.getReceivedDataBit(15)).divide(this.bigDecimals.get("BIG_DECIMAL_100f"), 2, BigDecimal.ROUND_CEILING);
        pumpSettings.totalDailyDose = AnimasUtils.createBigDecimalValueThroughMoreBits(packet.getReceivedDataBit(16),
            packet.getReceivedDataBit(17)).divide(this.bigDecimals.get("BIG_DECIMAL_100f"), 2, BigDecimal.ROUND_CEILING);
        ...
        if ((this.data.isModelPingOrHigher()))
        {
            pumpSettings.maxDoseIn2h = AnimasUtils.createBigDecimalValueThroughMoreBits(packet.getReceivedDataBit(18),
                packet.getReceivedDataBit(19)).divide(this.bigDecimals.get("BIG_DECIMAL_100f"), 2, BigDecimal.ROUND_CEILING);
            startBit = 20;
        }
        else
        {
        ...
        if ((this.data.isModel1200()) || (this.data.isModel1200p()))
        {
            ...
            this.data.addSettingTimeValueEntry(new SettingTimeValueEntry(AnimasSettingSubType.InsulinCHRatio, 1, this
                    .calculateTimeFromTimeSet(0), i2c_s3));
            this.data.addSettingTimeValueEntry(new SettingTimeValueEntry(AnimasSettingSubType.InsulinCHRatio, 2, this
                    .calculateTimeFromTimeSet(8), i2c_s0));
            this.data.addSettingTimeValueEntry(new SettingTimeValueEntry(AnimasSettingSubType.InsulinCHRatio, 3, this
                    .calculateTimeFromTimeSet(20), i2c_s1));
            this.data.addSettingTimeValueEntry(new SettingTimeValueEntry(AnimasSettingSubType.InsulinCHRatio, 4, this
                    .calculateTimeFromTimeSet(32), i2c_s2));
            this.data.addSettingTimeValueEntry(new SettingTimeValueEntry(AnimasSettingSubType.InsulinCHRatio, 5, this
                    .calculateTimeFromTimeSet(44), i2c_s3));
        }
        else
        {
            BigDecimal sickBGOverLimit = AnimasUtils.createBigDecimalValueThroughMoreBits(packet.getReceivedDataBit(startBit + 10),
                packet.getReceivedDataBit(startBit + 11));
            ...
            {
                pumpSettings.sickDaysCheckOverLimit = sickBGOverLimit.divide(
                    bigDecimals.get("BIG_DECIMAL_18f"), 1, BigDecimal.ROUND_CEILING);
            }
    ...
    private void decodeBasalProfile(AnimasDevicePacket adp)
    {
        ...
        for (int p = 0; p < profileEntries; p++)
        {
            ...
            BigDecimal amount = AnimasUtils.createBigDecimalValueThroughMoreBits(adp.getReceivedDataBit((20 + (p * 2))),
                adp.getReceivedDataBit((20 + (p * 2) + 1))).divide(bigDecimals.get("BIG_DECIMAL_1000f"), 3,
                BigDecimal.ROUND_CEILING);
    ...
    private void decodeDosingSettings(AnimasDevicePacket adp)
    {
        for (int i = 0; i < 6; i++)
        {
            this.data.addSettingTimeValueEntry(new SettingTimeValueEntry(AnimasSettingSubType.InsulinCHRatio, (i + 1),
            ...
            this.data.addSettingTimeValueEntry(new SettingTimeValueEntry(AnimasSettingSubType.InsulinBGRatio, (i + 1),
        ...
        for (int i = 0; i < 12; i++)
        {
            ...
            this.data.addSettingTimeValueEntry(new SettingTimeValueEntry(AnimasSettingSubType.BGTarget, (i + 1), this
                    .calculateTimeFromTimeSet(i * 8), this.data.isBGinMgDL() ? bgTarget : (float) (bgTarget / 18.0f),
    ...
    private void decodeInsulinBGRatio(AnimasDevicePacket adp) // 39/5
    {
        ...
        for (int i = 0; i < maxEntry; i++)
        {
            ...
            if (this.data.isBGinMgDL())
            {
                this.data.addSettingTimeValueEntry(new SettingTimeValueEntry(AnimasSettingSubType.InsulinBGRatio,
    ...
    private void decodeBGTarget(AnimasDevicePacket adp) // 39/6
    {
        ...
        for (int i = 0; i < maxEntry; i++)
        {
            ...
            if (this.data.isBGinMgDL())
            {
                this.data.addSettingTimeValueEntry(new SettingTimeValueEntry(AnimasSettingSubType.BGTarget, (i + 1),
                        this.calculateTimeFromTimeSet(adp.getReceivedDataBit((8 + i))), bgTarget, bgDelta));
            }
            else
            {
                this.data.addSettingTimeValueEntry(new SettingTimeValueEntry(AnimasSettingSubType.BGTarget, (i + 1),
                        this.calculateTimeFromTimeSet(adp.getReceivedDataBit((8 + i))), (bgTarget / 18.0f), (bgDelta / 18.0f)));
            }
        }
    }
    ...
    {
        this.data.pumpInfo.friendlyName = getStringFromPacket(adp, 6, 12).trim();
    }
