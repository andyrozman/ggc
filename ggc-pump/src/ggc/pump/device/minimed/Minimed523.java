...
public class Minimed523 extends Minimed522
{
    ...
    public void createCommands()
    {
        ...
        // CGMS SETTINGS
        util.addCommand(MinimedCommand.READ_SENSOR_PREDICTIVE_ALERTS, new MinimedCommand(209,
                "Read Sensor Predictive Alerts"));
        util.addCommand(MinimedCommand.READ_SENSOR_RATE_OF_CHANGE_ALERTS, new MinimedCommand(212,
                "Read Sensor Rate Of Change Alerts"));
        util.addCommand(MinimedCommand.READ_SENSOR_DEMO_AND_GRAPH_TIMEOUT, new MinimedCommand(210,
                "Read Sensor Demo and Graph Timeout"));
        ...
        // CGMS DATA
        util.addCommand(MinimedCommand.READ_VCNTR_HISTORY, new MinimedCommandHistoryCGMS(213, "Read Vcntr History",
                1024, 32));
    ...
    @Override
    public boolean convertCurrentSettings(MinimedCommand cmd) // throws
                                                              // BadDeviceValueException
    {
        ...
        util.config.addSetting("MM_SETTINGS_CAPTURE_EVENT_ENABLE",
            util.decoder.parseResultEnable(cmd.reply.raw_data[22]));
        util.config.addSetting("MM_SETTINGS_OTHER_DEVICE_ENABLE",
            util.decoder.parseResultEnable(cmd.reply.raw_data[23]));
        util.config.addSetting("MM_SETTINGS_OTHER_DEVICE_MARRIES_STATE",
            util.decoder.parseResultEnable(cmd.reply.raw_data[24]));
     ...
     * m_cmdReadVcntrHistoryData.m_maxRecords =
     * m_cmdReadGlucoseHistoryData.m_maxRecords;
     * m_cmdReadVcntrHistoryData.allocateRawData();
     * calcTotalBytesToRead();
     * m_cmdReadVcntrHistoryData.m_extraObject = new
     * IntRange(intrange.getMinimum(), intrange.getMaximum(),
     * intrange.getDefault(), "Vcntr Page Range");
     * log.info( (new StringBuilder()).append(
     * "decodeCurrentGlucoseHistoryPageNumber: , available Vcntr pages = "
     * ).append
     * (i).append(", Vcntr pages to read = ").append(m_cmdReadVcntrHistoryData
     * .m_maxRecords).toString());
     * }
     */
    ...
    protected boolean convertDebug(MinimedCommand cmd)
    {
        ...
        // log.debug("Low predictive alert - Time sensitivyty: 00:" +
        // (cmd.reply.raw_data[4] - Integer.parseInt("80", 16)));
        // log.debug("High predictive alert - Time sensitivyty: 00:" +
        // (cmd.reply.raw_data[3] - Integer.parseInt("80", 16)));
