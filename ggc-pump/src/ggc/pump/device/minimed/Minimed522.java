...
//      new MinimedCommand( 205, "Read Current Glucose History Page Number");
        ...
        // CGMS DATA COMMANDS
        util.addCommand(MinimedCommand.READ_GLUCOSE_HISTORY, new MinimedCommandHistoryCGMS(154, "Read Glucose History", 1024, 32));
        util.addCommand(MinimedCommand.READ_ISIG_HISTORY, new MinimedCommandHistoryCGMS(155,"Read Isig History", 2048, 32));
        

        // REMOVED COMMANDS
//        new MinimedCommand(40, "Write Glucose History Timestamp", 0);
//        new CommandHistoryDataFilter(168, "Filter Glucose History Data", (MM511.CommandHistoryData)m_cmdReadGlucoseHistoryData);
//        new CommandHistoryDataFilter(169, "Filter Isig History Data", (M511.CommandHistoryData)m_cmdReadIsigHistoryData);
...
//        log.debug("convertDeviceReply [code=" + mc.command_code + ",desc=" + mc.command_description + "]");
    ...
    */
    ...
    public String convertCalibrationFactor(MinimedCommand cmd)
    {
        ...
        double cal_f = (double)i / 1000.0d;
        //m_calibrationFactor = (double)i / 1000D;
        //log.info( (new StringBuilder()).append("decodeCalibrationFactor: factor=").append(m_calibrationFactor).toString());
    ...
    public boolean convertSensorSettings(MinimedCommand cmd)
    {
        ...
        util.config.addSetting("SENSOR_ENABLE", util.decoder.parseResultEnable(cmd.reply.raw_data[0]));
        ...
        util.config.addSetting("SENSOR_HIGH_GLUCOSE_SNOOZE_TIME", "" + hu.makeInt(cmd.reply.raw_data[4], cmd.reply.raw_data[5]));
        util.config.addSetting("SENSOR_LOW_GLUCOSE_LIMIT_ENABLE", util.decoder.parseResultEnable(cmd.reply.raw_data[6]));

        i = hu.makeInt(cmd.reply.raw_data[7], cmd.reply.raw_data[8]);
                
        if (bg_units == 2)
            util.config.addSetting("SENSOR_LOW_GLUCOSE_LIMIT_VALUE", "" + util.decoder.toWholeUnits(i));
        else
            util.config.addSetting("SENSOR_LOW_GLUCOSE_LIMIT_VALUE", "" + i);
        
        util.config.addSetting("SENSOR_LOW_GLUCOSE_SNOOZE_TIME", "" + hu.makeInt(cmd.reply.raw_data[9], cmd.reply.raw_data[10]));
