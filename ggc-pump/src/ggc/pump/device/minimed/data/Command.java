...
public class Command extends MMPCommand
{
    ...
    public void execute()  throws Exception
    {
        ...
        do
            try
            {
                MedicalDevice.logInfoLow(this, "execute: cmd=" + m_description + ", m_maxRetries=" + m_maxRetries + ", attempts=" + i);
            ...
            catch(BadDeviceCommException baddevicecommexception)
            {
                if(++i <= m_maxRetries)
                {
                    MedicalDevice.logWarning(this, "execute: cmd failed with exception: " + baddevicecommexception + "; retrying (attempts = " + (i + 1) + ")");
                    setState(7);
                    try
                    {
                ...
                {
                    MedicalDevice.logError(this, "cmd " + MedicalDevice.Util.getHex(m_commandCode) + " (" + m_description + ") failed after " + i + " attempts" + "; exception = " + baddevicecommexception);
                    throw new BadDeviceCommException("execute: cmd " + MedicalDevice.Util.getHex(m_commandCode) + " (" + m_description + ") failed after " + i + " attempts", baddevicecommexception.getErrorCode(), baddevicecommexception.getErrorCodeDescription());
                }
            }
...
//    Command(int command_code, String description, int rec_length, int max_records, int command_type)
//    Command(int command_code, String description, int rec_length, int max_records, int address, int address_length, int command_type)
