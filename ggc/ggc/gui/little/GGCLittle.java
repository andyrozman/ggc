...
*/
	...
	*/
	...
	menu.addSeparator();
	menu.add(createItem("EXIT_APP", "EXIT_APP_DESC", "exit_app"));
	menu.add(new JLabel());
	menu.add(new JLabel());
	...
	//item.setName(m_ic.getMessage(name));
	item.setToolTipText(m_ic.getMessage(desc));
...
*/
        ...
        public void actionPerformed(ActionEvent e)
        {
        	...
        	if (dailyStats.table.getSelectedRow()==-1) 
        	{
        	    JOptionPane.showMessageDialog(getMyParent(), m_ic.getMessage("SELECT_ROW_FIRST"), m_ic.getMessage("ERROR"), JOptionPane.ERROR_MESSAGE);
        	    return;
        	}

        	DailyValuesRow dvr = dailyStats.dayData.getRowAt(dailyStats.table.getSelectedRow());
        	...
        	try 
    	    	{
        	    dailyStats.dayData.deleteRow(dailyStats.table.getSelectedRow());
        	    dailyStats.model.fireTableChanged(null);
        	    m_db.saveDayStats(dailyStats.dayData);
    	    	}
    	    	catch (Exception ex) 
            ...
            {
        	DailyGraphDialog dgd = new DailyGraphDialog(getMyParent(), dailyStats.dayData);
        	dgd.setDailyValues(dailyStats.dayData);
            }
	...
	{
	    // not same date
	    JOptionPane.showMessageDialog(getMyParent(), 
		    m_ic.getMessage("CURRENT_DATE_HAS_CHANGED"), 
		    m_ic.getMessage("ERROR"), 
		    JOptionPane.ERROR_MESSAGE);
	    
	    this.dailyStats.refreshDayData();
