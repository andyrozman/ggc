...
public class MinimedCommandHistoryData extends MinimedCommand
{
    ...
    public void executeSubCommands(int start, int end) throws PlugInBaseException
    {
        ...
        //if(i <= j)
        {
            ...
            {
                log.info("executeSubCommands: (" + this.command_description + ") reading page " + k + " (reading up to page " + end + ")");
                flag = readPage(k);
            }
        ...
        /*else
        {
            ...
            {
                log.info( (new StringBuilder()).append("execute: (").append(this.command_description).append(") reading page ").append(l).append(" (reading down to page ").append(j).append(")").toString());
                flag = readPage(l);
            }

        }*/
        
        log.info( "executeSubCommands: " + this.command_description + " returned " + this.reply.raw_data.length + " bytes");
    }
    ...
    private boolean readPage(int i) throws PlugInBaseException
    {
        ...
        do
        {
            ...
            catch(PlugInBaseException ex)
            {
                ...
                {
                    log.warn( (new StringBuilder()).append("readPage: re-reading history data page ").append(i).append(" (attempts = ").append(j + 1).append(")").toString());
                } 
                else
                {
...
//            log.debug("MinimedCommandHistoryData");
            ...
            //m_cmdReadHistoryDataPage.command_parameters = utils.getParamatersArray(1, 0);
    ...
    MinimedDeviceUtil util = MinimedDeviceUtil.getInstance();
        ...
        void execute(int start_page, int end_page) throws PlugInBaseException
        {
            ...
            log.debug("execute: " + this.command_description + " returned " + this.reply.raw_data.length + " bytes");
        }
...
/*                    if(++j <= m_maxRetries)
      ...
      /*          }
            ...
            if(ai.length > 0)
                this.reply.raw_data = util.getHexUtils().concatIntArrays(this.reply.raw_data, ai);
        ...
        public MinimedCommandHistoryData(MinimedComm_Interface comm_int, int command_code, String description, int rec_length, int max_record)
        {
            ...
            int ai[] = new int[64];
            ai[0] = 0;
            m_cmdReadHistoryDataPage = new MinimedCommand(command_code, description + " (Page)", ai, 1);
