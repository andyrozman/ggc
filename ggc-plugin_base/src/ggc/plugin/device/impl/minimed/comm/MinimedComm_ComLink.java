...
public class MinimedComm_ComLink extends SerialProtocol implements MinimedComm_Interface //extends MinimedComm_Base
{
    ...
    private void initCommunications() throws PlugInBaseException, IOException //, SerialIOHaltedException
    {
...
/*
        ...
        try
        {
            ...
            param_int = Integer.parseInt(device.convertDeviceReply(mdc).toString());
        ...
        /*
        if (!checkCorrectReply(0, 67, 0, param_int, mdc.raw_data, m_ic.getMessage("MM_PUMP_ERROR_REPLY")))
        {
            log.error("Device is in error state. For communication to be succcesful device must be cleared of all errors !");
            return -1;
        }*/
...
/*
        ...
        if (!checkDeviceCorrectReply(0, 3,  util.getNormalPumpState(), param_int, mdc.reply.raw_data, m_ic.getMessage("MM_PUMP_STATE_REPLY")))
        {
        ...
        try
        {
            ...
            //param_int = this.util.decoder.getUnsignedShort(mdc, MinimedCommandReplyDecoder.TBR_DURATION);
        }
        catch(PlugInBaseException ex)
        {
            log.error("Exception: " + ex, ex);
  ...
  */      
    ...
    private void readMoreData()
    {
        ...
        int[] cmds = { 
    MinimedCommand.READ_BG_ALARM_CLOCKS, 
    MinimedCommand.READ_BG_ALARM_ENABLE,  
...
//            util.config.showSettings();
    ...
    public int closeDevice() throws PlugInBaseException
    {
        ...
        if (util.isCommandAvailable(MinimedCommand.CANCEL_SUSPEND))
        {
            ...
            if (util.config.firmware_version != null && (util.config.firmware_version.startsWith("VER 1.6") || util.config.firmware_version.startsWith("VER 1.7")))
            {
...
//        System.out.println("First nr: " + first_nr);
        ...
        {
            if (found_nak)
                throw new PlugInBaseException("readAckByte: reply " + this.hex_utils.getHex((byte)102) + " (NAK) does not match expected ACK reply of " + this.hex_utils.getHex((byte)85));
            else
                throw new PlugInBaseException("readAckByte: reply " + this.hex_utils.getHex(i) + " does not match expected ACK reply of " + this.hex_utils.getHex((byte)85));
        } 
        else
        {
            return true;
    ...
    public byte readByteTimeout(long ms) throws IOException, PlugInBaseException
    {
...
*/
    ...
    private void sendDeviceCommand(MinimedCommand command) throws PlugInBaseException, IOException, Exception
    {
        ...
        catch(IOException ex)
        {
            ...
            this.util.device_stopped_exception = new PlugInBaseException("sendCommand: ERROR - an IOException  has occurred processing " + command.getFullCommandDescription()  + ". Exception = " + ex);
        }
    }
    ...
    private void readDeviceReadyByteDevice(boolean send_byte) throws PlugInBaseException, IOException
    {
        ...
        if (i != 51)
            throw new IOException("readDeviceReadyByteDevice: reply " + hex_utils.getHex(i) + " does not match expected READY reply of " + hex_utils.getHex((byte)51));
        else
            return;
    }
    ...
    private boolean sendToDeviceCheckReply(byte cmd_byte, byte expected_return) throws IOException, PlugInBaseException
    {
        ...
        if (i != expected_return)
        {
            ...
            log.debug("SendCommand: command " + hex_utils.getHex(cmd_byte) + " reply " + hex_utils.getHex(i) + " does not match expected command " + hex_utils.getHex(expected_return));
            return false;
        } 
        else
        {
    ...
    private boolean checkDeviceCorrectReply(int min, int max, int required, int returned, int[] raw_data, String error_desc) throws PlugInBaseException
    {
...
/*
  ...
  */          
    ...
    /*
    public void sendCommandReadDataHistory(MinimedCommand command) throws PlugInBaseException
    {
        try
        {
            sendDeviceCommand(command);
    ...
    */
    ...
    private boolean checkHeaderAndCRC(MinimedCommand command, int arr_in[]) throws PlugInBaseException
    {
        ...
        if(this.util.config.serial_number_bcd[0] != arr_in[1] || this.util.config.serial_number_bcd[1] != arr_in[2] || this.util.config.serial_number_bcd[2] != arr_in[3])
    ...
    private boolean checkDeviceAcknowledge(MinimedCommand command) throws Exception
    {
        ...
        */
        ...
        if(ai1[4] != 6)
        {
            ...
            command.getReply().setException(new PlugInBaseException("checkAck: " + command.getFullCommandDescription() + " failed; error code = <" + hex_utils.getHex(j) + ">" + "(" + util.getNAKDescription(j) + ") " + "(byte data = " + "<" + hex_utils.getHex(ai1) + ">)"));
