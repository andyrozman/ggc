...
//    int special_entry_pump_map[] = { -1, 4, 4, 5 };
    ...
    static Hashtable<String, SpecialEntryDefinition> special_entries = null; //new Hashtable<String, String>(); 
    ...
    private void initSpecialEntries()
    {
        if (special_entries==null)
        {
            ...
            special_entries.put("1", new SpecialEntryDefinition(1, "URINE", "mmol/L", true, 4, ExtendedDailyValue.EXTENDED_URINE, SpecialEntryDefinition.TYPE_STRING));
            special_entries.put("2", new SpecialEntryDefinition(2, "URINE", "mg/dL", true, 4, ExtendedDailyValue.EXTENDED_URINE, SpecialEntryDefinition.TYPE_STRING));
            special_entries.put("3", new SpecialEntryDefinition(3, "URINE", "", false, 4, ExtendedDailyValue.EXTENDED_URINE, SpecialEntryDefinition.TYPE_STRING));
            special_entries.put("4", new SpecialEntryDefinition(4, "CH", "g", false, 5, -1, SpecialEntryDefinition.TYPE_DECIMAL_1));
            special_entries.put("-2", new SpecialEntryDefinition(-2, "BG", "", false, 3, -1, SpecialEntryDefinition.TYPE_INTEGER));
        }
    ...
    public String getValueValue()
    {
        ...
        {
            v = "" + da.getDecimalHandler().getDecimalAsString(da.getFloatValueFromString(this.special_entry_value.trim(), 0.0f)*1.0f, 1);
        }
        else if (special_entries.get("" + this.special_entry_id).type==SpecialEntryDefinition.TYPE_DECIMAL_2)
        {
            v = "" + da.getDecimalHandler().getDecimalAsString(da.getFloatValueFromString(this.special_entry_value.trim(), 0.0f)*1.0f, 2);
        }
        else 
        {
    ...
    {
        return DataAccessMeter.getInstance().getI18nControlInstance().getMessage(special_entries.get("" + this.special_entry_id).tag);
    }
    ...
    private class SpecialEntryDefinition
    {
        ...
        public SpecialEntryDefinition(int id_, String tag_, String unit_, boolean transfer_unit_, int pump_map_, int dv_ext_map, int type_ )
        {
            this.id = id_;
            this.tag = tag_;
            this.unit = unit_;
