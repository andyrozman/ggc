...
public class MeterDataHandler extends DeviceDataHandler
{
    ...
    public void executeExportDb_Old()
    {
         ...
         * database, now each
         * // plugin has it's own access to database.
         * // With glucose meters, that can measure also other stuff (like
         * urine) we also needed
         * // to rewrite this writing code a lot. Since this code has become
         * very complicated,
         * // there will be a lot of comments here.
         ...
         * // System.out.println("Size [add]: " + ll.get("ADD").size());
         * // System.out.println("Size [edit]: " + ll.get("EDIT").size());
         * Hashtable<Long,MeterValuesEntry> ht_add = new
         * Hashtable<Long,MeterValuesEntry>();
         * Hashtable<Long,MeterValuesEntry> ht_edit = new
         * Hashtable<Long,MeterValuesEntry>();
         * Hashtable<Long,ArrayList<MeterValuesEntry>> ht_add_edit = new
         * Hashtable<Long,ArrayList<MeterValuesEntry>>();
         * // this are time_marks for pump data retrieval
         * Hashtable<Long,String> time_marks = new Hashtable<Long,String>();
         * // we create separate hashtables, if we find same item we create
         * entry into special hashtable
         * // we leave base entry inside, so that we can easily find if we have
         * special cases
         * for ( DeviceValuesEntry doh : ll.get("ADD") )
         * {
         ...
         * this.element_count_all);
         * // we retrieve list of all pump reading
         * Hashtable<String, PumpDataExtendedH> pump_data =
         * db.getPumpData(time_marks);
         * this.setCustomStatus(1, 10);
         * //for(int i=0; i<2; i++)
         * for(int i=0; i<2; i++)
         * {
         ...
         * {
         * PumpDataExtendedH pdeh = null;
         * if (pump_data.containsKey(mve.getDateTime() + "_" +
         * mve.getPumpMappedType()))
         * {
         * // edit
         * pdeh = pump_data.get(mve.getDateTime() + "_" +
         * mve.getPumpMappedType());
         ...
         * } // for
         * }
         * // same records add_edit (first we go through all for same day and
         * add them as pump entries,
         * // then we merge them and write them to Pen/Injection mode
         * for(Enumeration<Long> en = ht_add_edit.keys(); en.hasMoreElements();
         * )
         * {
         ...
         * setCustomStatus(2, 0);
         * }
         * // merge special types and send them to hibernate with addedit option
         * (for meters) and
    ...
    public DeviceValuesTableModel getDeviceValuesTableModel()
    {
        ...
        // System.out.println("getDeviceValuesTableModel(): " +
        // mi.getInterfaceTypeForMeter() );
